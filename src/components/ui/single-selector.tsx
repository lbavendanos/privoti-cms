...
function removePickedOption(groupOption: GroupOption, picked: Option | null) {
  const cloneOption = JSON.parse(JSON.stringify(groupOption)) as GroupOption

  for (const [key, value] of Object.entries(cloneOption)) {
    cloneOption[key] = value.filter((val) => picked?.value !== val.value)
  }
  return cloneOption
}
  ...
  (
    ...
    const selectables = React.useMemo<GroupOption>(
      () =>
        removePickedOption(
          options,
          selected ? { value: selected, label: '' } : null,
        ),
      [options, selected],
    )
          ...
          <div className="relative flex flex-wrap gap-1">
            ...
            <button
              ...
              className={cn(
                ...
                (hideClearButton || disabled || selected === null) && 'hidden',
              )}
            >
              <X />
            </button>
